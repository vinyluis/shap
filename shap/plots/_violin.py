...
def violin(shap_values, features=None, feature_names=None, max_display=None, plot_type="violin",
    ...
    plot_size : "auto" (default), float, (float, float), or None
        ...
        unchanged. Ignored if `ax` is passed.
            ...
            for pos, i in enumerate(feature_order):
                ...
                for i in range(len(xs) - 1):
                    if ds[i] > 0.05 or ds[i + 1] > 0.05:
                        ax.fill_between([xs[i], xs[i + 1]], [pos + ds[i], pos + ds[i + 1]],
                                        [pos - ds[i], pos - ds[i + 1]], color=colors.red_blue_no_bounds(smooth_values[i]),
                                        zorder=2)

        else:
            parts = ax.violinplot(shap_values[:, feature_order], range(len(feature_order)), points=200, vert=False,
                                  widths=0.7,
    ...
    if color_bar and features is not None and plot_type != "bar" and \
        ...
        m.set_array([0, 1])
        cb = pl.colorbar(m, ax=ax, ticks=[0, 1], aspect=80)
        cb.set_ticklabels([labels['FEATURE_VALUE_LOW'], labels['FEATURE_VALUE_HIGH']])
        cb.set_label(color_bar_label, size=12, labelpad=0)
        cb.ax.tick_params(labelsize=11, length=0)
        cb.set_alpha(1)
        cb.outline.set_visible(False)
        # bbox = cb.ax.get_window_extent().transformed(ax.dpi_scale_trans.inverted())
